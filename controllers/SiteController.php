...
class SiteController extends FrontendbaseController
{
    ...
    {   
        ...
        $Taskmodel = new Task();
        $query = new\yii\db\Query();
        $tasks = $query->select(['{{%order}}.*', '{{%spare_parts}}.*'])
            ->from('{{%spare_parts}}')
            ->join('inner JOIN', '{{%order}}', '{{%order}}.order_id = {{%spare_parts}}.task_order_id')
            ->where(
                [
                    'order_status' => [101, 102 ,103 , 104],
                    'task_status' => [101, 102, 103, 104, 105, 106, 107,111],
                ]
            )
            ->limit(31)
            ->orderBy(
    ...
    {
        $message_check = yii::$app->request->post('yzm');
