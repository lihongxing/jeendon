...
class GlobalHelper
{
    ...
    static function generate_order_number($type){
        list($usec, $sec) = explode(" ", microtime());
        $str  = rand(10,99);
        switch($type){
            case 3:
                $number = 'I'. date("YmdHis").$str;;
                break;
            case 4:
                $number = 'O'. date("YmdHis").$str;;
                break;
        }
        return $number;
    }
     ...
     * @return string
     */
    static function generate_order_demand_number($type){
        list($usec, $sec) = explode(" ", microtime());
        $str  = rand(10,99);
        foreach (ConstantHelper::$order_type_details as $order_type_detail){
            foreach ($order_type_detail['types'] as $type1){
                if($type1['val'] == $type){
                    $number = $type1['flag'].date("YmdHis").$str;
                    break;
                }
            }
        }
     ...
     * @return string
     */
    static function generate_task_order_number(){
        list($usec, $sec) = explode(" ", microtime());
