...
$this->title = Yii::t('admin', 'emp-debitrefund-list');
            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <div class="panel-body">
                        <form id="form1" role="form" class="form-horizontal" method="get" action="<?=Url::toRoute('/admin/emp-debitrefund/emp-debitrefund-list')?>">
                            <div class="form-group">
                                <label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">消息发送方式</label>
                                <div class="col-sm-8 col-lg-9 col-xs-12">
                                    <div class="btn-group">
                                        <a href="<?=Url::toRoute(['/admin/emp-debitrefund/emp-debitrefund-list','debitrefund_status' =>100])?>" class="btn btn-<?= $GET['debitrefund_status'] == 100 ? 'primary' : 'default'?>">未处理</a>
                                        <a href="<?=Url::toRoute(['/admin/emp-debitrefund/emp-debitrefund-list','debitrefund_status' =>101])?>" class="btn btn-<?= $GET['debitrefund_status'] == 101 ? 'primary' : 'default'?>">处理中</a>
                                        <a href="<?=Url::toRoute(['/admin/emp-debitrefund/emp-debitrefund-list','debitrefund_status' =>102])?>" class="btn btn-<?= $GET['debitrefund_status'] == 102 ? 'primary' : 'default'?>">已处理</a>
                                        <a href="<?=Url::toRoute(['/admin/emp-debitrefund/emp-debitrefund-list','debitrefund_status' =>''])?>" class="btn btn-<?= $GET['debitrefund_status'] == '' ? 'primary' : 'default'?>">全部</a>
                                    </div>
                                </div>
                                <input type="hidden" name="debitrefund_status" value="<?= $GET['debitrefund_status']?>" />
                            </div>
                            <div class="form-group">
                                <label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">工程师信息</label>
                                <div class="col-sm-8 col-lg-9 col-xs-12">
                                    <input placeholder="可搜索工程师用户名，手机号码等" value="<?= $GET['keyword']?>" name="keyword" class="form-control" type="text">
                                </div>
                            </div>
                            <div class="form-group">
                    ...
                    <table class="table table-bordered">
                        ...
                        <?php if (!empty($emp_debitrefund_list)) { ?>
                            <?php foreach ($emp_debitrefund_list as $key => $item) { ?>
                                <tr class="odd gradeX">
                                    <td>
                                    ...
                                    <td>
                                        <?=date('Y年m月d日 H时m分',$item['debitrefund_add_time']) ?>
                                    </td>
                                    <td>
                                        <?php
                                        switch($item['debitrefund_status']){
                                            case 101:
                                    ...
                                    <td>
                                        <a class="btn btn-default btn-sm checkbox-toggle" href="<?=Url::toRoute(['/admin/emp-debitrefund/emp-debitrefund-detail','debitrefund_id' => $item['debitrefund_id']])?>" type="button">
                                            <i class="fa fa-eye"></i>
                                            详情
                                        </a>
                                    </td>
                        ...
                        <?php } else { ?>
                            <tr class="odd gradeX">
                                <td style ="text-align: center" colspan="7">当前未添加任何退款扣款！</td>
                            </tr>
                        <?php } ?>
                        </tbody>
                    </table>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        switch(data.status){
                            ...
                            case 102:
                            case 103:
                                content = '雇主退款扣款申请删除失败';
                                break;
                            case 403:
                                content = '你没有删除雇主退款扣款申请权限';
                                break;
                        }
                        dialog({
                            title: prompttitle,
