...
class SiteController extends AdminbaseController
{
    ...
    public function actionWelcome()
    {
        ...
        $employercount = Employer::find()
            ->count();
        $engineercount1 = Engineer::find()
            ->count();
        $engineercount3 = Engineer::find()
            ->where([
                'eng_examine_type' => [2, 3],
            ])
            ->sum('eng_member_num');
        $engineercount = $engineercount3 + $engineercount1;
	 ...
	 /**
      * 获取网站最近15日的访问量
	  * @params type 1：欢迎页面数据
      * @return json 访问量数据
      */
	public function actionGetVisitsCount()
	{
		$type = yii::$app->request->post('type');
		if($type == 1){
			$nowDayVisitCounts = Visit::find()
				->count();
			$visitDaysCounts = VisitCount::find()
			...
			$visitDays = array();
			$visitcounts = array();
			if(!empty($visitDaysCounts)){
				for($i=count($visitDaysCounts)-1; $i>=0; $i--){
					array_push($visitDays, date('m-d',strtotime($visitDaysCounts[$i]['created_time'])));
					array_push($visitcounts, intval($visitDaysCounts[$i]['nums']));
				}
			}
			array_push($visitDays, date('m-d'));
			array_push($visitcounts, intval($nowDayVisitCounts));
			return $this->ajaxReturn(array('visitDays' => $visitDays, 'visitcounts' => $visitcounts));
		}
	}
    ...
    /**
     * 网站站点基本配置
     * 1：需求发布时，工艺需求，结构需求是否显示
     * 2；
     */
    public function actionConfigure()
    {
        if(yii::$app->request->isPost){
            $configure = yii::$app->request->post('configure');
            $file = "../config/configure.php";
            $str = "<?php return " . var_export($configure, TRUE) . ";?>";
            $count = file_put_contents($file, $str);
            if($count > 0){
                $this->success("站点建设站点配置成功！");
            }else{
                $this->error("站点建设站点配置失败");
            }
        }else{
            return $this->render('configure',[
                'configure' => yii::$app->params['configure']
            ]);
        }
    }
}
