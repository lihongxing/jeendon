(function(window) {
		...
		if(!elm.editor) {
			require(['editor'], function(){
				var editor = tinyMCE.createEditor(id, {
					...
					toolbar2: "code print | styleselect fontsizeselect link image multiimage media emoticons 'print', 'preview', 'searchreplace', 'drafts', 'help' ",
					language: 'zh_CN',
	...
	util.iconBrowser = function(callback){
		...
		var modalobj = util.dialog('请选择图标',['/admin/site/icon.html?c=utility&a=icon&callback=selectIconComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
