...
$(".asx").mouseleave(function(){
	...
	var fname = '';//字段名
	switch(fewd){
		case 0:fname="order_achievements";break;
		case 1:fname="order_design_software_version";break;
		case 2:fname="order_whether_parameter";break;
		case 4:fname="order_part_standard";break;
		case 5:fname="order_bidding_period";break;
	}
...
$('.asxta').mouseleave(function(){
	...
	text1 = "<input type='hidden' name='"+fname+"' id='"+fname+"' value='"+text1+"'>";
	$('#nexta').children('div').eq(fewf).find('.fit').eq(fewo).empty().html(text+text1);
	$(this).hide();
})
//工序别多选判定
...
$("body").on('click','.filedi',function(e){
	...
	var pageY = e.pageY;
	$('#filedi').css({'position':'absolute','top':pageY-290,'margin-left':'635px','z-index':'101'});
	if($('#eventdit').val()=='data0'){
		$('#filedi').css({'position':'absolute','top':pageY-290,'margin-left':'0px','z-index':'101'});
	}
	//设置上传后保留结果值的地方
...
function wrongd(bb,cc){
	var dd = '';
	switch(cc){
		case 110:dd = '还未填写';break;
		case 120:dd = '输入格式不正确';break;
	}
