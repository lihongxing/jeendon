...
$this->title = Yii::t('app', 'empconductinglisttitle');
$this->registerMetaTag(array(
    "name" => "keywords", "content" => Yii::t('app', 'empconductinglistkeywords')
));
$this->registerMetaTag(array(
    "name" => "description", "content" => Yii::t('app', 'empconductinglistdescription')
));
?>
...
<!--            <th>待确认任务总数</th>-->
        ...
        <?php if(!empty($conductingorders)){?>
            <?php foreach($conductingorders as $key => $item){?>
                <tr>
...
<!--                    </td>-->
                    ...
                    <td>
                        <a onclick="window.location.href = '<?=Url::toRoute(['/emp-order-manage/emp-conducting-order-detail', 'order_id' => $item['order_id']])?>'" class="btn btn-success btn-xs"><i class="fa fa-fw fa-eye"></i>查看</a>
                    </td>
                <tr>
            <?php }?>
        <?php }else{?>
