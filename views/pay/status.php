    ...
    <div class="outp" style="background: rgba(0, 0, 0, 0) url('/frontend/images/loiva.png') no-repeat scroll 27px 4px">
        ...
        <p style="font-size: 16px">
            <?php if(!isset($message)){?>
                您的订单号为<span style="margin-left: 0;color: #666;font-size: 18px"> <?=$order['order_number']?> </span>的订单已成功付款，付款金额为<span style="font-size: 18px;margin-left: 0"> <?=$order['order_pay_total_money']?> </span>元，可点击立即跳转来查看订单详细信息</p>
            <?php }else{?>
                <?=$message?>
            <?php }?>
        <a href="<?= $status == 'ok' ? \yii\helpers\Url::toRoute('/emp-order-manage/emp-conducting-order-list') : \yii\helpers\Url::toRoute(['emp-order-manage/emp-order-pay', 'order_id' => $order['order_id']])?>"><input value="立即跳转" type="button"></a>
    </div>
</div>
