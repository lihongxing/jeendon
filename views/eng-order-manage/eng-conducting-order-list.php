...
$this->title = Yii::t('app', 'engconductinglisttitle');
$this->registerMetaTag(array(
    "name" => "keywords", "content" => Yii::t('app', 'engconductinglistkeywords')
));
$this->registerMetaTag(array(
    "name" => "description", "content" => Yii::t('app', 'engconductinglistdescription')
));
?>
        ...
        <?php if(!empty($orderofferlist)){?>
            <?php foreach($orderofferlist as $key => $item){?>
                <tr>
                    ...
                    <td>
                        <button href="#" onclick="window.location.href = '<?=Url::toRoute(['/eng-order-manage/eng-order-conducting-offer-detail', 'offer_id' => $item['offer_id']])?>'" class="btn btn-info btn-xs"><i class="fa fa-fw fa-eye"></i>查看</button>
                    </td>
                </tr>
            <?php }?>
        <?php }else{?>
