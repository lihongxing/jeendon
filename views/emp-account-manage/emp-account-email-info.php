...
<?php if($flag == 1){?>
<div id="Bphone">
    <form action="<?=Url::toRoute('/emp-account-manage/emp-account-email-info')?>" method="post" id="DzhiM" class="DzhiM">
        <ul>
            <li>
            ...
            <li style="text-align: center;"><input type="submit" class="Opisf" name="" value="立即认证"></li>
        </ul>
...
<?php }elseif($flag == 2){?>
<div id="Bphone">
    <form action="<?=Url::toRoute('/emp-account-manage/emp-account-email-info')?>" method="post" id="DzhiM1" class="DzhiM">
        <ul>
            <li>
                <div class="Umingc">邮箱号：</div>
                <input id="" name="email" readonly value="<?=yii::$app->employer->identity->emp_email?>" class="Jujsr_1" type="text" placeholder="">
            </li>

            <li style="text-align: center;"><input type="submit" class="Opisf" name="" value="解除绑定"></li>
        </ul>
        <input type="hidden" value="<?=$flag?>" name="flag">
        <input type="hidden" name="_csrf" id="csrftoken" value="<?=yii::$app->request->getCsrfToken()?>">
    </form>
        ...
        $("#DzhiM").validate({
            rules: {
                email: {
                    ...
                    email: true,
                    remote:{
                        url:"<?=Url::toRoute('/emp-account-manage/emp-account-email-check')?>",//后台处理程序
                        data:{
                            _csrf:function(){
