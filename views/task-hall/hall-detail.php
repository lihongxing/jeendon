...
<div id="cheng">
    ...
    <div id="putg" class="fl">
        ...
        <ul class="Send">
            ...
            <li class="card_g"><?= $task['task_part_type'] ?><?php if($task['order_type'] == '结构图纸设计'){?>  &nbsp;&nbsp;<?=$task['task_totalnum']?>套<?php }?> &nbsp;&nbsp;模具<?= $task['order_type'] ?></li>
            <li class="card4">
        ...
        <?php }else if($task['task_status'] >= 102 && $task['task_status'] <= 107){?>
            <div id="Barfn">
                ...
                <ul class="Nhf">
                    <li class="marg_1"><?= date('Y-m-d', $task['order_add_time']) ?></li>
                    <li class="marg_2"><?= date('Y-m-d', $task['order_add_time']) ?></li>
                    <li class="marg_3"><?= date('Y-m-d', $task['order_pay_time'] < ($task['order_bidding_period'] * 3600 * 24 + $task['order_add_time']) ? $task['order_pay_time'] : $task['order_bidding_period'] * 3600 * 24 + $task['order_add_time']) ?></li>
            ...
            <div class="pion">
                ...
                <table class="outie">
                                        ...
                                        <?php if(!empty($offer)){?>
                                            <?php if($offer['offer_status'] == 100){?>
                                                <a href="<?=$task['task_parts_number_mold']?>">
                                                    <img src="/frontend/images/yansuo.png">点击下载
                                                </a>
                                            <?php }else{?>
                                                仅中标可下载
                                            <?php }?>
                    ...
                    <tr class='biaot'>
                        ...
                        <td colspan='2'>招标状态</td>
                    </tr>
                    <?php if(!empty($task['procedures'])){$k=0;?>
                        <?php foreach($task['procedures'] as $l => $item){?>
                            <tr>
                                <?php if($task['order_type'] == '结构图纸设计'){?>
                                    <td><?=$item['task_process_id']?></td>
                                    <td  colspan='2'><?=$item['task_process_name']?></td>
                                    <td><?=$item['task_source_pressure']?></td>
                                <?php }?>
                                <td><?=$item['task_duration']?></td>
                                <?php if($l == 0){?>
                                    <td colspan='2' rowspan="<?=$task['task_totalnum']?>"><?=date('Y-m-d',$task['order_add_time'])?></td>
